...
class Pose:
    ...
    def draw(self, img, img_id, pose_id):
...
#                 if part_id == 3 or part_id == 5 or part_id == 14 or part_id == 16:
#                     cv2.circle(img, (int(x_b), int(y_b)), 3, [255, 0, 255], -1) #[255, 0, 255]
#                 else:
                cv2.circle(img, (int(x_b), int(y_b)), 3, Pose.color, -1)
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), Pose.color, 2)
        _, ear_r = self.keypoints[BODY_PARTS_KPT_IDS[14][1]]
        _, ear_l = self.keypoints[BODY_PARTS_KPT_IDS[16][1]]
        _, wrist_r = self.keypoints[BODY_PARTS_KPT_IDS[3][1]]
        _, wrist_l = self.keypoints[BODY_PARTS_KPT_IDS[5][1]]
        
        if (ear_r > wrist_r+6) and (ear_l > wrist_l+6) and (self.bbox[3] > 150) and (ear_r>0) and (ear_l>0) and (wrist_r>0) and (wrist_l>0):
            cv2.imwrite("processed/frame_{}_{}.jpg".format(img_id, pose_id), img[max(0, self.bbox[1] -30) : min(self.bbox[1] + self.bbox[3]+ 30, img.shape[0]), max(0, self.bbox[0]  -30): min(self.bbox[0] + self.bbox[2] + 30, img.shape[1]),:])
